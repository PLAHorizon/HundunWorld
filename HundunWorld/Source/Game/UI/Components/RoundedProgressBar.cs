...
namespace HundunWorld.Game.UI.Components
{
    ...
    public class RoundedProgressBar : ProgressBar
    {
        ...
        public override void Draw()
        {
            ...
            // 考虑进度条边距
            float margin = BarMargin.Left;
            barRect.X += margin;
            barRect.Y += margin;
            barRect.Width -= margin * 2;
            barRect.Height -= margin * 2;
            
            // 计算实际进度条宽度
            var progressWidth = barRect.Width * Value;
            barRect.Width = progressWidth;
            ...
            if (Value > 0.01f)
            {
                var barRadius = CornerRadius;
                if (barRect.Width < CornerRadius * 2)
                    barRadius = barRect.Width / 2f;
                
                DrawRoundedBox(barRect, BarColor, barRadius);
            }
